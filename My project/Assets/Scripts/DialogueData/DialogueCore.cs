...
using System.Collections.Generic; // 需添加此命名空间
public class DialogueCore : MonoBehaviour
{
    ...
    public void StartDialogue(DialogueDataSO dialogueData)
    {
        ...
        if (uiController != null) uiController.InitDialogueUI();
    ...
    public void PlayNextDialogueLine()
    {
        ...
        if (nextLine != null)
        {
            ...
            // 有下一句，刷新UI显示
            if (uiController != null) uiController.RefreshDialogueUI(nextLine);
        }
        else
        {
            // 没有下一句，对话结束
    ...
    private void EndDialogue()
    {
        ...
        if (uiController != null) uiController.CloseDialogueUI();
    ...
    private void Update()
    {
        ...
        if (isDialoguePlaying && Input.GetMouseButtonDown(0))
        {
            ...
            // 如果点击的是按钮，让按钮自己处理（不切对话）
            if (results.Count > 0 && results[0].gameObject.GetComponent<UnityEngine.UI.Button>() != null)
            {
                return;
            }
